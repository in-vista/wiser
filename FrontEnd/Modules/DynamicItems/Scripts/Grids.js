                ...
                dataSource: {
                    ...
                    transport: {
                        ...
                        update: async function(transportOptions) {
                            ...
                            const updateQueryId = gridViewSettings.updateQueryId;
                            if(updateQueryId) {
                                updateResult = await Wiser.api({
                                    url: `${window.dynamicItems.settings.wiserApiRoot}items/${encodeURIComponent(data.encrypted_id)}/action-button/0?queryId=${encodeURIComponent(updateQueryId)}`,
                                    method: "POST",
                                    contentType: "application/json",
                                    dataType: "json",
                                    ...
                                    kendo.alert(`Er is iets fout gegaan tijdens het opslaan van het veld '${data.name}'.<br>` + (errorThrown ? errorThrown : 'Probeer het a.u.b. nogmaals, of neem contact op met ons.'));
                                ...
                                updateResult = await Wiser.api({
                                    ...
                                    console.error("UPDATE FAIL", textStatus, errorThrown, jqXHR);
                                    kendo.alert(`Er is iets fout gegaan tijdens het opslaan van het veld '${itemModel.title}'.<br>` + (errorThrown ? errorThrown : 'Probeer het a.u.b. nogmaals, of neem contact op met ons.'));
                                    transportOptions.error(jqXHR);
                                });
                            }

                            transportOptions.success(updateResult);
